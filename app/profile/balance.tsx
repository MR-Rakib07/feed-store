...
import {
  ...
  Alert, ActivityIndicator, FlatList, Modal, TouchableWithoutFeedback
} from 'react-native';
  ...
  if (loading) {
    return (
      <View className="flex-1 justify-center items-center bg-slate-50">
        ...
        <Text className="text-slate-500 font-semibold mt-2.5 text-xs">ব্যালেন্স লোড হচ্ছে...</Text>
      </View>
    );
  }
    ...
    <SafeAreaView className="flex-1 bg-slate-50 px-4 pt-3" edges={['bottom']}>
      <View className="bg-white rounded-3xl p-5 border border-slate-100 shadow-sm mb-4">
        <View className="flex-row justify-between items-center mb-1">
          ...
          <View className={`px-2.5 py-0.5 rounded-full ${netBalance <= 0 ? 'bg-blue-50' : 'bg-rose-50'}`}>
            <Text className={`text-[10px] font-extrabold uppercase tracking-wide ${netBalance <= 0 ? 'text-blue-700' : 'text-rose-700'}`}>
              {netBalance < 0 ? 'অগ্রিম জমা' : netBalance === 0 ? 'পরিশোধিত' : 'মোট বকেয়া'}
            </Text>
          </View>
        </View>

        <Text className={`text-3xl font-black tracking-tight mb-4 ${netBalance <= 0 ? 'text-blue-600' : 'text-rose-600'}`}>
        ...
        <View className="flex-row justify-between pt-3 border-t border-slate-100">
          <View className="flex-1 pr-2">
            ...
            <Text className="text-sm font-black text-slate-800 mt-0.5" numberOfLines={1}>
          ...
          <View className="flex-1 items-end pl-2">
            ...
            <Text className="text-sm font-black text-emerald-700 mt-0.5" numberOfLines={1}>
      ...
      <View className="flex-row gap-x-3 mb-4">
        <TouchableOpacity
          ...
          className="flex-1 bg-emerald-600 h-12 rounded-2xl flex-row justify-center items-center shadow-sm shadow-emerald-200 active:bg-emerald-700"
          activeOpacity={0.8}
        >
          <Feather name="plus-circle" size={16} color="#fff" />
          <Text className="text-white font-extrabold text-xs uppercase tracking-wider ml-1.5">
            টাকা জমা দিন
          </Text>
        </TouchableOpacity>
        ...
        <TouchableOpacity
          ...
          className="flex-1 bg-rose-600 h-12 rounded-2xl flex-row justify-center items-center shadow-sm shadow-rose-200 active:bg-rose-700"
          activeOpacity={0.8}
        >
      ...
      <View className="flex-1">
        ...
        <FlatList
          ...
          renderItem={({ item }) => {
            ...
            return (
              <View className="bg-white p-3.5 rounded-2xl border border-slate-100 mb-2.5 shadow-sm">
                <View className="flex-row justify-between items-center">
                  <View className="flex-row items-center flex-1 pr-2">
                    <View className={`w-9 h-9 rounded-xl justify-center items-center mr-3 ${isPayment ? 'bg-emerald-50' : 'bg-rose-50'}`}>
                    ...
                    <View className="flex-1">
                      <Text className="text-xs font-bold text-slate-900 leading-4" numberOfLines={1}>
                  ...
                  <View className="flex-row items-center gap-x-1.5">
                    <Text className={`text-sm font-black mr-1 ${isPayment ? 'text-emerald-700' : 'text-rose-600'}`}>
                      {isPayment ? `- ৳ ${formatCurrency(item.amount)}` : `+ ৳ ${formatCurrency(item.amount)}`}
                    </Text>
                    <TouchableOpacity
                      onPress={() => handleOpenEditModal(item)}
                      className="p-1.5 bg-slate-50 hover:bg-emerald-50 active:bg-emerald-100 rounded-lg"
                      activeOpacity={0.7}
                    >
                    ...
                    <TouchableOpacity
                      ...
                      className="p-1.5 bg-slate-50 hover:bg-rose-50 active:bg-rose-100 rounded-lg"
                      activeOpacity={0.7}
                    >
                ...
                {item.note ? (
                  <View className="mt-2.5 pt-2 border-t border-slate-100 flex-row items-start bg-slate-50/70 p-2 rounded-xl">
                    <Text className="text-[10px] font-extrabold text-slate-400 uppercase mr-1.5 mt-0.5">নোট:</Text>
          ...
          ListEmptyComponent={
            <View className="py-14 items-center justify-center">
              <Text className="text-slate-400 text-xs font-semibold">কোনো লেনদেনের রেকর্ড নেই।</Text>
            </View>
          }
        />
      </View>
        ...
        <TouchableWithoutFeedback onPress={closeModal}>
          <View className="flex-1 bg-black/30 justify-center items-center px-6">
            <TouchableWithoutFeedback>
              <View className="bg-white rounded-3xl p-5 w-full border border-slate-100 shadow-xl">
                ...
                <Text className="text-xs text-slate-400 font-medium mb-4 leading-4">
                ...
                <View className="mb-3">
                  <Text className="text-xs font-bold text-slate-700 uppercase mb-1.5 tracking-wider">
                    টাকার পরিমাণ (৳)
                  </Text>
                  <TextInput
                    ...
                    className="bg-slate-50 border border-slate-200 focus:border-emerald-500 rounded-xl h-11 px-3 text-sm font-bold text-slate-900"
                  />
                </View>
                ...
                <View className="mb-4">
                  ...
                  <TextInput
                    ...
                    className="bg-slate-50 border border-slate-200 focus:border-emerald-500 rounded-xl h-11 px-3 text-xs text-slate-900"
                  />
                </View>
                ...
                <View className="flex-row gap-x-2.5">
                  <TouchableOpacity
                    onPress={closeModal}
                    className="flex-1 bg-slate-200 h-11 rounded-xl justify-center items-center active:bg-slate-300"
                    activeOpacity={0.8}
                  >
                    <Text className="text-slate-700 font-bold text-xs">বাতিল</Text>
                  </TouchableOpacity>
                  <TouchableOpacity
                    onPress={handleSaveTransaction}
                    disabled={submitting}
                    className={`flex-1 h-11 rounded-xl justify-center items-center shadow-sm ${actionType === 'payment' ? 'bg-emerald-600 shadow-emerald-200 active:bg-emerald-700' : 'bg-rose-600 shadow-rose-200 active:bg-rose-700'}`}
                    activeOpacity={0.8}
                  >
                    {submitting ? (
