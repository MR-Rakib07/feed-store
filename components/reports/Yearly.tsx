...
export default function YearlyReportScreen() {
  ...
  const report = useMemo(() => {
    ...
    // ২. সরাসরি জমা ও বকেয়ার হিসাব (Payments)
  ...
  if (loading) {
    return (
      <View className="flex-1 justify-center items-center bg-slate-50">
        ...
        <Text className="text-slate-500 font-semibold mt-3 text-sm">বার্ষিক রিপোর্ট তৈরি হচ্ছে...</Text>
      </View>
    );
  }
  ...
  return (
    <ScrollView className="flex-1 bg-slate-50 p-4" showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 100 }}>
      ...
      <View className="bg-emerald-600 rounded-3xl p-5 mb-4 shadow-lg shadow-emerald-200">
        <Text className="text-emerald-100 text-xs uppercase font-extrabold tracking-widest">মোট বার্ষিক খরচ ও বকেয়া ({selectedYear})</Text>
      ...
      {/* Category Breakdown */}
      <Text className="text-slate-900 font-extrabold text-base mb-3">ক্যাটাগরি অনুযায়ী বিবরণ ({selectedYear})</Text>
      ...
      {categoryList.length > 0 ? (
        ...
        <View className="py-10 bg-white rounded-2xl border border-dashed border-slate-200 items-center justify-center">
          ...
          <Text className="text-slate-400 font-medium text-xs mt-2">{selectedYear} সালের কোনো হিসাব পাওয়া যায়নি।</Text>
        </View>
      )}
